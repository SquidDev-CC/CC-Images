-- Optimsed buffer for CC (uses CC's colours instead)
...
return function(depthTest, dimensions)
	--[[ Various TODOs:
	...
	insert(builder, "local function clearColour(colour) colour = colour or 'f' for i = 1, " .. size .. " do colours[i] = colour end end\n")
	insert(builder, "export.clearColour = clearColour\n")

	-- Setup colour buffer
	insert(builder, "clearColour('f')\n")
	...
	insertVars [=[
	local colour_lookup = colors.strSets
